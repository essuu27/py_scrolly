#!python
"""
py-scrolly: v1.0 - A screen "toy" that manipulates text and graphics onscreen.

It requires the pygame module [https://www.pygame.org/].
"""
...
# The 'effects' directory holds any 'extra' text effects. Find them and load them dynamically.
...
def screendrift(screen_x, screen_y):
    ...
    if (tmpx == 0) or (tmpx == 7):
        screen_x -= 1
        screen_y -= 1
    elif (tmpx == 1) or (tmpx == 6):
        screen_x += 1
        screen_y -= 1
    elif (tmpx == 2) or (tmpx == 5):
        screen_x += 1
        screen_y += 1
    elif (tmpx == 3) or (tmpx == 4):
        screen_x -= 1
        screen_y += 1
...
def footer(my_canvas, font2):
    ...
    # Get 'text' Surface dimesions, centre on X-axis and move 50 units from the bottom.
...
def main():
    ...
    # Load in the background music. The distributed tune is 'Mellow strain' by U4iaF8 (thanks Jim!)
    # However, the script will find any MP3 file in the directory and play it.
    ...
    if pg.font.match_font("rubik"):
        ...
        if os.path.exists("rubik.ttf"):
            ...
            font1 = pg.font.SysFont("Calibri", 48)
    ...
    # config.scrollwid holds much much horizontal distace is left until the end of message line
    ...
    clock = pg.time.Clock()
    # mouse_time holds the last time the mouse pointer moves
    ...
    while running:
        ...
        # Draw the scrolling message onto the canvas
        my_canvas, text_img = effectmods[effect_selected].effect(my_canvas, text_img)

        if text_scrolling:
            # Shift the text image left by 2 pixels, new message handling subroutine needed
...
#        screen.blit(pg.transform.smoothscale(my_canvas, canvas_res), canvas_xy)
